<template>
  <q-layout view="lHh Lpr lFf">
    <q-page-container>
      <q-page>
        <q-footer>
          <q-btn :label="nomeBtn" flat text-color="white" class="full-width bg-primary" size="20px" no-caps
            @click="salvar()" />
        </q-footer>

        <div class="row justify-center">
          <div style="width: 800px; max-width: 90vw;">
            <div class="row q-gutter-sm bg-white text-primary"
              style="margin-left: -15px; padding-top: 9px; padding-bottom: 10px;">
              <div class="col-1">
            ...
            <div class="row justify-center text-center text-subtitle1"
              ...
              <label>Informe seus dados para realizar o cadastro. Esses dados são utilizados exclusivamente para que
                você seja identificado em nossa plataforma, você poderá realizar a exclusão desses dados a qualquer
                momento.</label>
            </div>

            <div class="row q-gutter-sm" style="margin: 0px" v-if="passo === 1">
              <q-input v-model="nome" ref="nome" label="Nome" placeholder="Seu nome ou nome da sua empresa"
                class="full-width" :input-style="{ fontSize: '16px' }" outlined @keyup.enter="proximo('telefone')" />

              <q-input v-model="telefone" ref="telefone" label="Telefone" placeholder="(99) 99999-9999"
                class="full-width" @keyup="formataTelefone()" :input-style="{ fontSize: '16px' }" outlined
                @keyup.enter="proximo('descricao')"
                v-if="tornarPrestador === undefined || tornarPrestador === null || tornarPrestador === false" />

              <q-toggle v-model="whatsapp" label="Esse telefone tem Whatsapp?" checked-icon="mdi-whatsapp"
                color="primary" icon-color="white" size="80px" left-label style="margin-left: 2px"
                v-if="tornarPrestador === undefined || tornarPrestador === null || tornarPrestador === false" />

              <q-input v-model="descricao" ref="descricao" label="Descrição do negócio"
                placeholder="Descreva aqui as informações relevantes sobre o seu negócio" type="textarea" autogrow
                class="full-width" :input-style="{ fontSize: '16px' }" outlined @keyup.enter="proximo('cep')"
                v-if="tipoCadastro === 'p'" />

              <q-input v-model="cep" ref="cep" label="CEP" placeholder="CEP do seu endereço ou da sua empresa"
                class="full-width" type="number" :input-style="{ fontSize: '16px' }" @keyup="buscaCep()" maxlength="8"
                outlined @keyup.enter="proximo('continuar')"
                v-if="tornarPrestador === undefined || tornarPrestador === null || tornarPrestador === false" />

              <q-input v-model="cidadeUf" label="Cidade" class="full-width" disable
                :input-style="{ fontSize: '16px', fontWeight: '600' }" outlined
                v-if="tornarPrestador === undefined || tornarPrestador === null || tornarPrestador === false" />
            </div>

            <div class="row q-gutter-sm" style="margin: 0px" v-if="passo === 2 && tipoCadastro === 'p'">
              <div class="row scroll">
                <q-input v-model="pesquisar" ref="pesquisar" placeholder="Quais serviços você presta?"
                  input-class="text-center" class="q-ml-md full-width"
                  :input-style="{ fontSize: '18px', color: '#333' }" style="margin-bottom: 20px" outlined
                  @keyup="filtrar()">
                ...
                <q-list class="full-width" v-if="listaCategoria.length > 0">
                  <template v-for="categoria in listaCategoria" :key="categoria._id">
                    <q-item class="list-item" style="padding-left: 16px; padding-right: 16px;">
                      ...
                      <q-item-section>
                        <q-item-label
                          class="text-subtitle1 text-weight-bold text-grey-9">{{ categoria.nome }}</q-item-label>
                      </q-item-section>
                    </q-item>
                    ...
                    <q-list class="full-width">
                      <template v-for="subcategoria in categoria.subCategorias" :key="subcategoria._id">
                        <q-item class="list-item" style="padding-left: 16px; padding-right: 16px;" tag="label">
                          <q-item-section avatar style="padding-right: 10px !important; min-width: 0px !important;">
                            <q-checkbox v-model="categoriasSelecionadas" :val="subcategoria._id" />
                          </q-item-section>
            ...
            <div class="row q-gutter-sm" style="margin: 0px"
              v-if="passo === 3 && (tornarPrestador === undefined || tornarPrestador === null || tornarPrestador === false)">
              <q-input v-model="usuario" ref="usuario" label="CPF ou CNPJ" class="full-width"
                :input-style="{ fontSize: '16px' }" @keyup="formataCpfCnpj()" @keyup.enter="proximo('senha')"
                maxlength="18" outlined />

              <q-input v-model="senha" ref="senha" label="Senha" :type="isPwd ? 'password' : 'text'" class="full-width"
                outlined @keyup.enter="proximo('confirmaSenha')">
                <template v-slot:append>
                  <q-icon :name="isPwd ? 'mdi-eye-off-outline' : 'mdi-eye-outline'" class="cursor-pointer"
                    @click="isPwd = !isPwd" />
                </template>
              </q-input>

              <q-input v-model="confirmaSenha" ref="confirmaSenha" label="Confirmar senha"
                :type="isPwd2 ? 'password' : 'text'" class="full-width" outlined>
                <template v-slot:append>
                  <q-icon :name="isPwd2 ? 'mdi-eye-off-outline' : 'mdi-eye-outline'" class="cursor-pointer"
                    @click="isPwd2 = !isPwd2" />
                </template>
              </q-input>
            </div>
